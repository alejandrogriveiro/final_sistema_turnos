...
def validar_telefono(telefono):
    ...
    patron = r'^[0-9\-\s\+()]+$'
    return re.match(patron, telefono) is not None and len(telefono.replace(" ", "").replace("-", "")) >= 8
...
def solicitar_datos_paciente():
    ...
    # Solicitar nombre
    while True:
        nombre = input(centrar_texto("Ingrese nombre (0 para cancelar): ")).strip().title()
        if nombre == "0":
            return None
        if not nombre:
    ...
    # Solicitar apellido
    while True:
        apellido = input(centrar_texto("Ingrese apellido (0 para cancelar): ")).strip().title()
        if apellido == "0":
            return None
        if not apellido:
    ...
    # Solicitar email
    while True:
        email = input(centrar_texto("Ingrese email (0 para cancelar): ")).strip().lower()
        if email == "0":
            return None
        if not email:
...
def alta_paciente():
    ...
    # Solicitar DNI
    while True:
        dni = input(centrar_texto("Ingrese DNI (7-8 dígitos) o 0 para volver: ")).strip()
        if dni == "0":
            return
        if not dni:
    ...
    if guardar_pacientes(pacientes):
        print(centrar_texto(f"✅ Paciente {datos['nombre']} {datos['apellido']} registrado correctamente"))
    else:
...
def modificar_paciente():
    ...
    # Solicitar DNI
    dni = input(centrar_texto("Ingrese DNI del paciente a modificar (0 para volver): ")).strip()
    if dni == "0":
        return
    ...
    limpiar_pantalla()
    print(centrar_texto("=== DATOS ACTUALES ==="))
    ...
    nuevos_datos = solicitar_datos_paciente()
    ...
    pacientes[dni] = nuevos_datos
    
    if guardar_pacientes(pacientes):
        print(centrar_texto("✅ Paciente modificado correctamente"))
    else:
...
def eliminar_paciente():
    ...
    # Solicitar DNI
    dni = input(centrar_texto("Ingrese DNI del paciente a eliminar (0 para volver): ")).strip()
    if dni == "0":
        return
    ...
    confirmacion = input(centrar_texto("¿Confirma la eliminación? (s/n): ")).strip().lower()
    
    if confirmacion == 's':
        # Verificar si tiene turnos asignados
        from turnos import verificar_turnos_paciente
        if verificar_turnos_paciente(dni):
            print(centrar_texto("❌ No se puede eliminar. El paciente tiene turnos asignados"))
            pausar()
            return
